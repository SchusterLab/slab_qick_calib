...
##                if cp > delta and f[i] > (min(f)+2*fwindow):
    ...
    R_Q_list = np.zeros(len(peaklist))
    ...
    for i in range(len(peaklist)):
        ...
        R_Q_list[i] = abs(a_list[i])*Qr_list[i]**2 / Qc_hat_mag_list[i]



        if pdf_rewrite == True:
            # print(f_curr[0],f_curr[-1])
    ...
    plt.figure()
    plt.hist(R_Q_list,bins=np.logspace(2,6,50))
    # plt.gca().set_yscale('log')
    plt.gca().set_xscale('log')
    plt.ylim(0,6.25)
    plt.xlabel(r'$R_Q$')
    plt.title(r'$R_Q$ histogram')
    plt.show()
